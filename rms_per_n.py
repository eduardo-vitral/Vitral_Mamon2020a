...
# Gets the numercial calculations
nu_num   = np.load('dens_num1000.npy')
M_num    = np.load('mass_num1000.npy')

# Gets the density grid for different models and for the numerical deprojection
#nu_num = getNU(eta,m)
nu_SP  = getNUSimm(eta,m)
nu_EG  = getNUEG(eta,m,'cubic',True)
nu_LN  = getNUm(eta,m,'LN')

# Gets the density for r = R_e
BIG = 1e30
nu_1 = getNU(1,m)

# Gets the mass grid for different models and for the numerical deprojection
#M_num    = getM(eta,m)
M_SP     = getMSimm(eta,m)
M_LN     = getMm(eta,m,'LN')

# Saves the results from the numerical calculations to be used after on
np.save('mass_num1000', M_num) 
np.save('dens_num1000', nu_num)
...
# Gets the model proposed on Vitral & Mamon 2020a
nu_numRat = nu_LN/nu_num
nu_VM2Rat = 10**(np.log10(nu_numRat) - Polynomial(eta,m,coeff_nu))

M_numRat = M_LN/M_num
M_VM2Rat = 10**(np.log10(M_numRat) - Polynomial(eta,m,coeff_M))

#---------------------------------------------------------------------------

fig, axes = plt.subplots(figsize=(10, 6))
colors = ['red', 'b', 'limegreen', 'black']
model  = ['\mathrm{LGM}', '\mathrm{SP}', '\mathrm{EV}', '\mathrm{new}']
func   = [nu_LN/nu_num,nu_SP/nu_num,nu_EG/nu_num,nu_VM2Rat]
funcM  = [M_LN/M_num,M_SP/M_num,0,M_VM2Rat]
...
for i in range(0,len(colors)):
    ...
    for j in range(0,len(m)) :
        ...
        # Rejects points where density < density(r-R_e) / BIG
...
legend1 = plt.legend([lines[i] for i in [5,6]], [r"$f = \rho$", 
                     "$f = M$"], loc=3,
                     prop={'size': 18})
legend2 = plt.legend([lines[i] for i in [0,2,4,5]], ["$\mathrm{model}: " + \
                     str(model[i]) + "$" for i in range(4)], loc=1, 
                     prop={'size': 18})
axes.add_artist(legend1)
axes.add_artist(legend2)    
plt.xlabel(r'$n$', fontsize = 18)
plt.ylabel(r"$\mathrm{rms} \ \log " + \
           r"\left( \widetilde{f}_{\mathrm{model}} \, /" + \
...
print('Index n where EV08 is better than our density model:', 
      round(m[idx_4:len(rms_arrays[4])][idx_EV],1))
